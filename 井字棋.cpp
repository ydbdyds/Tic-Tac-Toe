...
static player p1, p2;
int s[3][3], tf = 0, gofirst = 1, row, column, over;
bool jieshu = false;
...
int main()
{
	...
	cout << "小帅哥小美女快来玩啊" << endl;

	cout << "电脑模式（0），结束游戏（1），双人模式（2）: ";
	cin >> over;
	while (over == 0)
	{
		cout << "你想选择什么难度" << endl << "简单模式（0） 普通模式（1） ";
		int mode;
		cin >> mode;
		jieshu = false;
		for (int x = 0; x < 3; x++)
			for (int y = 0; y < 3; y++) s[x][y] = 0;
		cout << "你想先走(1) 还是让电脑先走(0): ";
		cin >> gofirst;
		if (gofirst == 1) tf = 0;
